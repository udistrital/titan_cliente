        ...
        self.ver_detalle_persona = function () {
          if ($scope.contrato != undefined && $scope.preliquidacion != undefined) {
            ...
            if ($scope.preliquidacion.NominaId.ParametroPadreId.Nombre === "CT"){
              titanMidRequest.get('detalle_preliquidacion', '/obtener_detalle_CT/' + $scope.preliquidacion.Ano + '/' + $scope.preliquidacion.Mes + '/' + $scope.contrato.NumeroContrato+ '/' + $scope.contrato.Documento).then(function (response) {
